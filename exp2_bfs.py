...
used_id=[]
no=0
with open("file0.csv", "w", encoding="utf-8", newline="") as f:
...
def bdbk(no,name,numm=-1):
    ...
    with open("file0.csv", "a", encoding="utf-8", newline="") as f:
        csv_writer = csv.writer(f)
...
def bfs(name,num=-1):
    ...
    while queue:
        ...
        if nn in used_id:
            continue
        time.sleep(0.5)
        lists,realn,flag=bdbk(no,na,nn)
        if flag:
            print('第',str(no),'个:',na,'completed!')
            used_id.append(int(realn))
            no+=1
        else:
            print('第',str(no),'个:',na,'failed!')
        if len(lists)==0:
            continue
        for list in lists:
